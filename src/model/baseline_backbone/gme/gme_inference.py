...
class GmeQwen2VL(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.learnable_token = nn.Parameter(torch.randn(1, 1, self.base.config.hidden_size))
    ...
    def forward(
        ...
        **kwargs
        ...
        ## ---------------------------------------------------------------------
        # # concatenating learnable token
        # batch_size = inputs_embeds.size(0) # input_embeds: [B, T, D]
        # learnable_token = self.learnable_token.expand(batch_size, -1, -1)  # [B,1,D]
        # inputs_embeds = torch.cat([inputs_embeds, learnable_token], dim=1)  # [B, T+1, D]

        # # attention mask expansion
        # if attention_mask is not None:
        #     token_mask = torch.ones(batch_size, 1, device=attention_mask.device, dtype=attention_mask.dtype)
        ...
        if left_padding:
            if self.pooling == 'average':
                ...
                embeddings = outputs.hidden_states[self.chosen_layer][:, -1]
        else:
            sequence_lengths = pooling_mask.sum(dim=1) - 1
            batch_size = outputs.last_hidden_state.shape[0]
            if self.pooling == "average":
                embeddings = outputs.last_hidden_state[self.chosen_layer][torch.arange(
                    batch_size, device=outputs.last_hidden_state.device
                ), sequence_lengths].mean(dim=1)
            else:
                embeddings = outputs.hidden_states[self.chosen_layer][torch.arange(
                    batch_size, device=outputs.last_hidden_state.device
                ), sequence_lengths]
        if self.normalize:
    ...
    def set_chosen_layer(self, chosen_layer):
        self.chosen_layer = chosen_layer if chosen_layer else -1

    def set_pooling(self, pooling):
        self.pooling = pooling if pooling else 'last'
