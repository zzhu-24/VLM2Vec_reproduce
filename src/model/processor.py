...
#     for text, images in zip(texts, visual_inputs):
#         # --- (1) Normalize text and image ---
#         if not isinstance(text, str) or len(text.strip()) == 0:
#             text = "Describe the image."

#         # convert single PIL.Image into list
#         if isinstance(images, PIL.Image.Image):
#             images = [images]
#         # remove None from image list
#         if isinstance(images, list):
#             images = [im for im in images if isinstance(im, PIL.Image.Image)]
#         if len(images) == 0:
#             images = None
...
#                 )
#             else:
#                 # ensure text contains visual token
#                 if vlm_image_token not in text and vlm_video_token not in text:
#                     text = f"{vlm_image_token} {text}"
...
#             # --- (3) Extract and validate input_ids ---
#             input_id = inputs.get("input_ids")
#             if input_id is None:
#                 # fallback: minimal valid token (e.g., BOS only)
#                 from numpy import array
#                 input_id = array([[processor.tokenizer.bos_token_id]])
...
#                 pixel_values.append(None)
#                 image_grid_thw.append(None)
#                 pixel_values_videos.append(inputs.get('pixel_values_videos', None))
#                 video_grid_thw.append(inputs.get('video_grid_thw', None))

#         except Exception as e:
#             print(f"[Qwen2_VL_process_fn] failed on sample. text={text[:80]!r}, error={e}")
#             raise e
        
#     # # --- DEBUG BEGIN ---
#     # for i, ids in enumerate(input_ids):
#     #     if ids is None:
#     #         print_master(f"[DEBUG] ⚠️ sample {i} has input_ids=None, text={texts[i]!r}")
#     #     elif not isinstance(ids, (list, tuple)):
#     #         print_master(f"[DEBUG] sample {i} has strange input_ids type: {type(ids)}")
#     #     elif len(ids) == 0:
#     #         print_master(f"[DEBUG] ⚠️ sample {i} has empty input_ids, text={texts[i]!r}")
#     #     else:
#     #         print_master(ids)
#     # # --- DEBUG END ---
...
#     # --- (5) Pad ---
#     batch_encoding = processor.tokenizer.pad({'input_ids': input_ids}, return_tensors="pt")
#     input_ids, attention_mask = batch_encoding['input_ids'].long(), batch_encoding['attention_mask'].long()
